...
build_fixtures() {
  ...
  # tests/data/covtail.vcf.gz is COMMITTED, not generated here. It used to be
  # built at run time by an awk program using srand(23)/rand(), and awk's rand()
  # is implementation-defined: the same program and seed produce a different
  # file under BWK awk (macOS), gawk, and mawk (the default awk on Ubuntu, and
  # so on the CI runners). The goldens were captured under BWK awk, and the two
  # covtail cases failed on Linux with a different resolved sample size -- and a
  # different set of output COLUMNS, since nSNPsUsed appears only when a site is
  # actually excluded. Every other fixture here is either committed or derived
  # deterministically from committed data; this one was the exception.
  #
  # What the file contains, which is what the two cases depend on: 2000 sites,
  # 8 samples, 3% missing genotypes except at four sites that are ~75%
  # uncalled. That gives a minimum per-site n of 4 and a 99.9% target of 6,
  # with 2 sites below it -- so the file is large enough for the quantile rule
  # to differ from "the n every site can reach". The 400-site fixtures cannot
  # exercise that: 0.1% of 400 rounds to index 0.
...
define_case covtail-default   table -- --vcf "$HERE/data/covtail.vcf.gz" --sites --winsize 200 --winstep 200 --pi --s --d --h
define_case covtail-strict    table -- --vcf "$HERE/data/covtail.vcf.gz" --sites --winsize 200 --winstep 200 --pi --s --d --h --target-n 4
